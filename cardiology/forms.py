...
class PatientUploadForm(forms.ModelForm):
    # Field name matches the 'ecg' Modality slug -- see
    # templates/common/upload/modalities/ecg.html, which accesses it as
    # `patient_upload_form.ecg`, the same convention laparoscopy's `video` uses.
    ecg = forms.FileField(
        required=False,
    ...
    def __init__(self, *args, user=None, current_project=None, domain=None, **kwargs):
        ...
        if user:
            projects_qs = Project.objects.filter(domain=CARDIOLOGY_DOMAIN, is_active=True)
