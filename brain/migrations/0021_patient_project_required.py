...
Companion to 0020_folder_project_required, which did the same for folders.
``Patient.project`` was added as nullable so the folder->project migration
(0018_alter_folder_unique_together_remove_patient_folders_and_more) could backfill
it, and stayed nullable, which left it optional in every ModelForm -- the Django
admin's "add patient" page included. A project-less patient is invisible to the
patient list, the export builder and every permission check that resolves rights
through the project.
...
* a patient that already has a folder takes **that folder's** project, since the
  ...
  0020_folder_project_required); assigning the catch-all instead would leave the
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("brain", "0020_folder_project_required"),
