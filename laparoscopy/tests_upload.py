...
class LaparoscopyUploadValidationTests(TestCase):
    def setUp(self):
        self.project, _ = Project.objects.get_or_create(slug="laparoscopy", defaults={"name": "Laparoscopy", "domain": "laparoscopy"})
    ...
    def test_empty_upload_does_not_create_patient(self):
        response = self.client.post(
            reverse("laparoscopy:upload_patient"),
            {"name": "Empty video", "project": str(self.project.id), "folder": str(self.folder.id)},
        )
