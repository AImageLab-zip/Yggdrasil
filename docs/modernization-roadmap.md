...
6. **Pre-existing brain API auth gap (found in Phase 7 mapping, NOT introduced here; independent of the demo — the demo never routes to `brain/api_views.py`).** Needs a dedicated PR + maintainer decision. `brain/api_views.py` endpoints (all wired in `brain/app_urls.py:188-222`) have NO auth:
   - `serve_file` (:80) — anyone can fetch any brain file by id (used by the authed viewer via `{% url ns:':api_serve_file' %}`, so adding `@login_required` is safe for the frontend, unlike the unauthed callers). **But no ACL/`user_can_read_folder`** either, so authed cross-folder read is also open — mirror maxillo `files.serve_file`.
   - `get_file_registry` (:65) — leaks brain file paths + patient ids. `get_job_status` (:34) / `ProcessingJobListView` (:14) — leak job state.
   - `runner_claim_job`/`runner_complete_job`/`runner_fail_job` (:41-62) — **unauthenticated POST that mutates brain job state, no token auth** (unlike maxillo's frozen runner contract at `maxillo/api_views/runner.py`). Overlaps **risk #1 (frozen runner contract)** — do NOT change these without confirming whether an external runner actually hits `brain/api/runner/...` and what token scheme it must use. Note `runner_claim_job` calls `get_job_status` internally, so `get_job_status` cannot simply be `@login_required` without breaking the runner path. This is why it was left untouched in Phase 7.
