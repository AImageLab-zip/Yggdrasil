...
class BrowserPanoramicTests(TestCase):
    def setUp(self):
        ...
        for user in (self.user, self.reader):
            ProjectAccess.objects.create(user=user, project=self.project, role="standard")
        self.folder = Folder.objects.create(name="Panorex cases")
        FolderAccess.objects.create(user=self.user, folder=self.folder, role="annotator")
        FolderAccess.objects.create(user=self.reader, folder=self.folder, role="standard")
        self.patient = Patient.objects.create(name="Panorex patient", folder=self.folder)
