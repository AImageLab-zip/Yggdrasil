...
EMAIL_HOST_USERS = [
    ...
    for username in (
        item.strip()
        for item in config("EMAIL_HOST_USERS", default=EMAIL_HOST_USER).split(",")
    )
    if username
]
...
if not EMAIL_HOST_USERS and DEFAULT_FROM_EMAIL:
    EMAIL_HOST_USERS.append(DEFAULT_FROM_EMAIL)
