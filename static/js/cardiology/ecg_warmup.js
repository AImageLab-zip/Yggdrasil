/**
 * Batch ECG-plot generation (administrators). Same mechanism as
 * static/js/panoramic_warmup.js: drive the normal patient view in a hidden
 * frame, one patient at a time; ecg_plot.js runs its unattended save pass
 * there and posts the outcome back, so there is no second copy of the
 * rendering logic to keep in sync.
 */
(function () {
    'use strict';

    var PATIENT_TIMEOUT_MS = 60000;
    ...
    function patientUrl(patientId) {
        var template = (window.ecgWarmup || {}).patientUrlTemplate || '';
    ...
    function scan() {
        ...
        setStatus('Looking for patients without an ECG plot...');

        var url = (window.ecgWarmup || {}).pendingUrl + '?folder=' + encodeURIComponent(folderId);
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(function (data) {
                ...
                if (!state.queue.length) {
                    setStatus('Every patient in ' + data.folder.name + ' already has an ECG plot.');
                    return;
                }
    ...
    function init() {
        ...
        window.addEventListener('message', function (event) {
            ...
            if (payload.type !== 'ecg-plot') return;
